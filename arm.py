...
class arm():
    def __init__(self, sweepMinBase = 130, sweepMaxBase = 40, angleMinBase = -pi/2, angleMaxBase = pi/2,
    			 sweepMinShoulder = 110, sweepMaxShoulder = 30, angleMinShoulder = pi/4, angleMaxShoulder = 3*pi/4,
    			 sweepMinElbow = 80, sweepMaxElbow = 45, angleMinElbow = pi/4, angleMaxElbow = -pi/4,
    			 sweepMinGripper = 40, sweepMaxGripper = 100, angleMinGripper = pi/2, angleMaxGripper = 0):
    ...
    def openGripper(self):
    	...
    	self.pwm.setPWM(self.gripper, self.angle2pwm("gripper", pi/4.0))
    	time.sleep(0.3)
